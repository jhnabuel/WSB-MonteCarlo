...
def monte_carlo_simulation_weekly_sentiment(stock, simulations, use_sentiment=True):
   ...
   # Generate both date ranges
    ...
    normalized_weekly_sentiment = normalize_sentiment(weekly_sentiment)
        ...
        # 2. Find the last available price before the forecast starts
    # (Assume your DataFrame is sorted by date)
    last_hist_idx = df[df["Date"] < forecast_start].index[-1]
    last_price = df.loc[last_hist_idx, "Close"]


     # Future prices
    simulated_prices = np.zeros((total_days + 1, simulations))
    for i in tqdm(range(simulations), desc=f"Simulating {simulations} paths"):
        simulated_prices[:, i] = simulate_single_path(
            last_price,
            total_days,
            base_drift,
            stdev,
    ...
    # Simulated price paths (skip the initial seed)
